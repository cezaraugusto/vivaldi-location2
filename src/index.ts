...
export default function locateVivaldi (
  ...
  const envPath = e?.VIVALDI_BINARY
...
const DEFAULT_INSTALL_STEPS: InstallGuidanceStep[] = [
  {
    summary: 'Install Vivaldi from the official site',
...
export function getInstallGuidance (opts?: InstallGuidanceOptions): string {
  ...
  lines.push('')
  lines.push('Alternatively, set VIVALDI_BINARY=/path/to/vivaldi and re-run.')

  return lines.join('\n')
}

export function locateVivaldiOrExplain (
  options?: boolean | {allowFallback?: boolean}
): string {
  const allowFallback =
    typeof options === 'boolean' ? options : Boolean(options?.allowFallback)

  const found = locateVivaldi(allowFallback) || locateVivaldi(true)
...
/**
 * Cross-platform Vivaldi version resolver.
 ...
 */
export function getVivaldiVersion (
  bin: string,
